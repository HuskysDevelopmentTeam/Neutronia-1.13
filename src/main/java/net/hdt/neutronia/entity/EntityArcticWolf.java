...
public class EntityArcticWolf extends EntityTameable
{
    ...
    public EntityArcticWolf(World p_i1696_1_) {
        super(NEntityTypes.ARCTIC_WOLF, p_i1696_1_);
        ...
        if (this.isTamed()) {
            if (!lvt_3_1_.isEmpty()) {
                if (lvt_4_1_ instanceof ItemFood) {
                    ...
                    if (lvt_5_1_.isWolfsFavoriteMeat() && (Float)this.dataManager.get(DATA_HEALTH_ID) < 20.0F) {
    ...
    public EnumDyeColor getCollarColor() {
        return EnumDyeColor.func_196056_a((Integer)this.dataManager.get(COLLAR_COLOR));
    }
    ...
    public EntityArcticWolf createChild(EntityAgeable p_createChild_1_) {
        EntityArcticWolf lvt_2_1_ = new EntityArcticWolf(this.world);
    ...
    public boolean canMateWith(EntityAnimal p_canMateWith_1_) {
        if (p_canMateWith_1_ == this) {
            ...
            return false;
        } else if (!(p_canMateWith_1_ instanceof EntityArcticWolf)) {
            return false;
        } else {
            EntityArcticWolf lvt_2_1_ = (EntityArcticWolf)p_canMateWith_1_;
            if (!lvt_2_1_.isTamed()) {
                return false;
    ...
    public boolean shouldAttackEntity(EntityLivingBase p_shouldAttackEntity_1_, EntityLivingBase p_shouldAttackEntity_2_) {
        if (!(p_shouldAttackEntity_1_ instanceof EntityCreeper) && !(p_shouldAttackEntity_1_ instanceof EntityGhast)) {
            if (p_shouldAttackEntity_1_ instanceof EntityArcticWolf) {
                EntityArcticWolf lvt_3_1_ = (EntityArcticWolf)p_shouldAttackEntity_1_;
    ...
    static {
        DATA_HEALTH_ID = EntityDataManager.createKey(EntityArcticWolf.class, DataSerializers.FLOAT);
        BEGGING = EntityDataManager.createKey(EntityArcticWolf.class, DataSerializers.BOOLEAN);
        COLLAR_COLOR = EntityDataManager.createKey(EntityArcticWolf.class, DataSerializers.VARINT);
    }

    class AIAvoidEntity<T extends Entity> extends EntityAIAvoidEntity<T> {
        private final EntityArcticWolf wolf;

        public AIAvoidEntity(EntityArcticWolf p_i47251_2_, Class<T> p_i47251_3_, float p_i47251_4_, double p_i47251_5_, double p_i47251_7_) {
        ...
        private boolean avoidLlama(EntityLlama p_avoidLlama_1_) {
            return p_avoidLlama_1_.getStrength() >= EntityArcticWolf.this.rand.nextInt(5);
        }
