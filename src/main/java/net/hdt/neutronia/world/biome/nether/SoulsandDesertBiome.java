...
public final class SoulsandDesertBiome extends Biome {
    ...
    public SoulsandDesertBiome() {
        super((new BiomeBuilder()).surfaceBuilder(new CompositeSurfaceBuilder<>(NETHER_SURFACE_BUILDER, SOULSAND_SURFACE)).precipitation(RainType.NONE).biomeCategory(Category.NETHER).depth(0.1F).scale(0.2F).temperature(2.0F).downfall(0.0F).waterColor(4159204).waterFogColor(329011).parent(null));
        this.addStructure(Feature.FORTRESS, new FortressConfig());
        this.setCarverForStage(Carving.AIR, createWorldCarverWrapper(NETHER_CAVE_WORLD_CARVER, new ProbabilityConfig(0.2F)));
        ...
        this.addCreature(EnumCreatureType.MONSTER, new SpawnListEntry(EntityType.GHAST, 50, 4, 4));
        this.addCreature(EnumCreatureType.MONSTER, new SpawnListEntry(EntityType.ZOMBIE_PIGMAN, 100, 4, 4));
        this.addCreature(EnumCreatureType.MONSTER, new SpawnListEntry(EntityType.MAGMA_CUBE, 2, 4, 4));
        this.addCreature(EnumCreatureType.MONSTER, new SpawnListEntry(EntityType.ENDERMAN, 1, 4, 4));
    }
}
